...
magnetic_field = 3500;  # magnetic field in G
...
class Parameters(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.timeconst.setCurrentIndex(tc_index)
        self.setfield.setValue(magnetic_field)
        ...
        self.setfield.valueChanged.connect(self.getValue_8)
    ...
    def getValue_7(self):
        ...
        return self.value    
    def getValue_8(self):
        self.value = self.setfield.value()    # Value of magnetic field
        return self.value

    # Submitting parameters to lock-in
    def submit(self):
        global frequency_1, mod_amplitude, time_constant, set_timer, number_scans, points, frequency_2, tc_index, magnetic_field
        ...
        magnetic_field = float(self.getValue_8())
        ...
        er031m.field_controller_set_field(magnetic_field)

    def ampl_calibration(self, ampl_g, f):

        a = 0.014854*math.sqrt(8.5*8.5 + 2*math.pi*2*math.pi*f*f*0.000537*0.000537)*ampl_g/(2*self.cal_function(f));        # 2 in denominator is empirical parameter to have the same value as in Bruker_Ver_6
        return a

    def cal_function(self, f):
        result = 0.978942 - 5.95114*f*f*10**(-13) - 7.442498*f*10**(-8) + 0.653611*math.exp(-0.0026463*f) + 0.304415*math.exp(-0.000476832*f) # calibration function
        return result
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.actionAdd_Scan.triggered.connect(self.change_scan_window)      ###########################################24.04.2020
    ...
    def cal_function(self, f):
        result = 0.978942 - 5.95114*10**(-13)*f*f - 7.44249*10**(-8)*f + 0.653611*math.exp(-0.0026463*f) + 0.304415*math.exp(-0.000476832*f)        # calibration function
        return result

    def new_point(self):
        global final_data_r, i, array_r, array_x, array_y, array_freq, j, start_time, end_time
        ...
        if j <= number_scans:
            ...
            if i < points and j == 1:
                ...
                sr_810.lock_in_modulation_frequency(freq_current)
                sr_810.lock_in_amplitude(self.ampl_calibration(mod_amplitude, freq_current))

                #data_r = sr_810.lock_in_signal()
                #########################################################24.04.2020
                data_x, data_y, data_r = sr_810.lock_in_signal_x_y_r()
                #print(sr_810.lock_in_amplitude())
                #frequency_1-step*i
                ...
                final_data_r = np.column_stack((array_freq, array_r, array_x, array_y))                                                   # appropriate format for pyqtgraph

                i = i + 1;
                end_time = time.time();
                self.lineTime.setText("Time (s): " + str(round((end_time - start_time), 1)) + "/" + str(round(number_scans*5 + time_constant*points*number_scans/1000,0)))    # time update
                ...
                array_y[i] = (array_y[i]*(j-1) + np.asarray(data_y))/j
            ...
            np.savetxt(file, np.array(['# '+ str(currenttime)  + ', \n' 'Frequency (Hz),' + 'Intensity (a. u.) R,' + 'Intensity (a. u.) X,' + 'Intensity (a. u.) Y']), fmt='%s')
    ...
    def plot(self, data1, data2, data3):
        ...
        pen2 = pg.mkPen(color = (0, 0, 255, 200)) # pen for plotting slightly transparent yellow line; the last argument is opacity; 255 - no transparancy; 0 - full transparancy
        pen3 = pg.mkPen(color = (255, 0, 0, 200)) # pen for plotting slightly transparent yellow line; the last argument is opacity; 255 - no transparancy; 0 - full transparancy
        ...
        self.graphicsView.plot(data3, pen = pen3)  # actual plotting and clear the previous one
    ...
    def updating(self):
        ...
        # plot new data
        self.plot(final_data_r[:, [0, 1]], final_data_r[:, [0, 2]], final_data_r[:, [0, 3]])

    # function to start timer for updating
    def start(self):
        global set_timer, array_r, array_x, array_y, array_freq, i, j, start_time, s
        ...
        if s == 0:
            ...
            message.show();
            message.buttonClicked.connect(self.clicked)
            return                              # stop current function

        start_time = time.time()
        #recalculation of timer for real time_constant used
        #set_timer = float(lock_in_time_constant());##################################################################################### check format of time constnat from lock-in
        array_x = [];
        array_y = [];
        array_r = [];
        array_freq = [];
    ...
    def clear_plot(self):
        ...
        if s == 0:
            ...
            message.setText("Data was not saved. Are you sure?    ");
            message.show();
            message.buttonClicked.connect(self.clicked_2)
            return
        else:
            self.graphicsView.clear()
            s = 1;
    ...
    def parameters_window(self):
        global frequency_1, mod_amplitude, tc_index, number_scans, points, frequency_2, magnetic_field

        frequency_1 = sr_810.lock_in_modulation_frequency()
        mod_amplitude_v = sr_810.lock_in_amplitude(self.ampl_calibration(mod_amplitude, frequency_1))
        tc_index = sr_810.lock_in_time_constant()
        points = points;
    ...
    def file_save(self, filename):
        ...
        try:
            if len (final_data_r) > 1:
                ...
                np.savetxt(file, np.array(['Frequency (Hz),' + 'Intensity R (a. u.),' + 'Intensity X (a. u.),' + 'Intensity Y (a. u.)']), fmt='%s')
                np.savetxt(file, final_data_r, delimiter=',', fmt='%.5e')
                file.close()
        except NameError:
                message = QMessageBox(self);
        ...
        path = os.path.dirname(filename) # for messxmorizing the path to the last used folder 
